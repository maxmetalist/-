...
def filter_by_state(id_list, state="EXECUTED"):
    """Функция фильтрации транзакций по параметру состояния"""
    filtered_list = []
    if not id_list:
        print("Укажите данные транзакции: id клиента, состояние, дата")
    else:
        for id_number in id_list:
            if id_number.get("state", 0) == "":
                return "Параметр состояния отсутствует"
...
def sort_by_date(id_list: Union[list, str], reverse: bool = True) -> Union[list, str]:
