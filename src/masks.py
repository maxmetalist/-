...
def get_mask_card_number(card_number: str) -> str:
    ...
    logger.debug("Проверка формата номера карты для создания маски")
    if len(card_number) == 16 and card_number.isdigit():
        logger.debug("Вывод результата")
        return card_number[:4] + " " + card_number[4:6] + "** **** " + card_number[-4:]
    else:
        logger.error("Не верный формат номера карты")
...
def get_mask_account(account_number: str) -> str:
    ...
    logger.debug("Проверка формата номера счёта для создания маски")
    if len(account_number) == 20 and account_number.isdigit():
        logger.debug("Вывод результата")
        return "**" + account_number[-4:]
    else:
        logger.error("Не верный формат номера счёта")
